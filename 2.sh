...
cd ..
	#京东农场
        ...
        sed -i "s/6632c8135/$new_fruit&/" JDScript/jd.sh

	#萌宠
        new_pet="MTE1NDUyMjEwMDAwMDAwNDI4ODA5NDU=@MTE1NDQ5OTUwMDAwMDAwNDI4ODA5NTE=@"
        #互助码填写格式：@+互助码    例如：@sd452a41ds2af1a1
        sed -i "s/MTE1NDAxN/$new_pet&/" JDScript/jd.sh
	...
	#种豆
        ...
        #互助码填写格式：@+互助码    例如：@sd452a41ds2af1a1
        sed -i "s/4npkonns/$new_plantBean&/" JDScript/jd.sh

	#京喜工厂
        new_dreamFactory="X2poJVLcLoygZX0TgGmkl8EiBIkQe_zrMAZqtgL24-M=@"
        #互助码填写格式：@+互助码    例如：@sd452a41ds2af1a1
        sed -i "s/4HL35B/$new_dreamFactory&/" JDScript/jd.sh

	#东东工厂
        new_jdfactory="T024anXulbWUI_NR9ZpeTHmEoPlACjVWnYaS5kRrbA@"
        #互助码填写格式：@+互助码    例如：@sd452a41ds2af1a1
        sed -i "s/P04z54XCjVWnYaS5m9cZ2f83X0Zl/$new_jdfactory&/" JDScript/jd.sh
	...
	#crazyJoy任务
        ...
        #互助码填写格式：@+互助码    例如：@sd452a41ds2af1a1
        sed -i "s/rHYmFm9wQ/$new_crazyJoy&/" JDScript/jd.sh

	#口袋书店
        new_jdbook="互助码填写位置@"
        #互助码填写格式：@+互助码    例如：@sd452a41ds2af1a1
        sed -i "s/d6d73edddaa64cbd/$new_jdbook&/" JDScript/jd.sh
